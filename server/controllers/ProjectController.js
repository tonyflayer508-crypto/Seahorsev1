...
// Add a revision request and return the updated project.
export async function chatProject(req, res) {
  ...
  project.messages.push({ role: 'user', content: prompt.trim() });
  project.messages.push({
    role: 'assistant',
    content: `Revision received: ${prompt.trim()}`,
  });
  project.version = String(Number(project.version || 0) + 1);
  project.status = 'completed';
  await project.save();
...
if(!req.user) {
  ...
  const projects = await Project.find(
    { owner: req.user.userId },
    {name: 1, description: 1, version: 1, updatedAt: 1, createdAt: 1}
  ).sort({ updatedAt: -1 });
...
// Update a project files (manual edits).
...
// Mark a project as publiciy published.
