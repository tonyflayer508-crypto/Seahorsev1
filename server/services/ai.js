...
// --- OpenRouter Models Client Setup ---
...
async function generateSingleFile(file, allFiles, prompt, alreadyGeneratedFiles) {
  ...
  if(code.trim().length === 0){
    ...
    // Applay post-generation validation and auto-fixing
...
const contextParts = [];
contextParts.push("## Current Project Files (mainfest)");
for await (const f of manifest) {
 ...
 if(Object.keys(relevantFiles).length > 0){
    ...
    for await (const [path, content] of Object.entries(relevantFiles)) {
    contextParts.push(`\n## ${path}\n\`\`\`\n${content}\n\`\`\``)
    
}
 }
 if(recentMessages.length > 0){
    contextParts.push("\n## File Recent Conversation");
...
if(rawParsed && Array.isArray(rawParsed.operations)){
    rawParsed.operations = rawParsed.operations.map((op)=>{
        ...
        else if (["update", "edit", "modify", "patch"].includes
        (opStr)) op.op = "update";
        else if (["delete", "remove", "del", "rm"].includes
        (opStr)) op.op = "delete";

        if(op.path && typeof op.path === "string" && !op.path.
            startsWith("/")){
            op.path = "/" + op.path;
            }
            ...
            if(op.op === "create" && op.content){
                const validation = validateRevisionContent(op.content,
                    op.path, "create");
                    op.content = validation.content;
                    if(validation.warnings.length > 0){
                        console.log(`[Validator] Revision Create adjustments
                        for ${op.path}:\n  -${validation.warnings.join("\n -")}`);
                    }
            }else if(op.op === "update" && op.replace){
          const validation = validateRevisionContent(op.replace,
                    op.path, "update");
                    op.replace = validation.content; 
                    if(validation.warnings.length > 0){
                     console.log(`[Validator] Revision Update adjustments
                    for ${op.path}:\n  -${validation.warnings.join("\n -")}`);
                    }
            }
            return op;
